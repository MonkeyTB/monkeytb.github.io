...
>
>3、自截断：这是论文稍微有意义的地方，自截断说白了就是随着解码过程，我们希望能碰到<eos>，这样就能停下来了，并且希望随着解码长度的增加碰到<eos>的概率也越来越多大，最终趋于1，这样就可以停下来了。定义$\color{blue}{p(<eos>\|y<t,x)=1-\alpha(h_t)}$，其中$\color{blue}{\alpha(h_0)=\sigma(w_{<eos>}^Th_0+b_{<eos>})}$、$\color{blue}{\alpha(h_t)=\sigma(w_{<eos>}^Th_t+b_{<eos>})[1-p(<eos>\|y<t-1,x)]}$
>
